...
describe("setupChannels workspace shadow exclusion", () => {
  ...
  it("re-enables a bundled plugin when the same channel is selected after going back", async () => {
    const configure = vi.fn(async ({ cfg, prompter }) => {
      await prompter.text({ message: "ClickClack token" });
      return { cfg };
    });
    ...
    const select = vi
      .fn()
      .mockResolvedValueOnce("clickclack")
      .mockResolvedValueOnce("clickclack")
      .mockResolvedValueOnce("__done__");
    const text = vi
      .fn()
    ...
    const onResolvedPlugin = vi.fn();

    const result = await setupChannels(
      { plugins: { allow: ["memory-core"] } } as OpenClawConfig,
      {} as never,
      {
        confirm: vi.fn(async () => true),
        note: vi.fn(async () => undefined),
    ...
    expect(result.plugins?.allow).toEqual(["memory-core", "clickclack"]);
    expect(result.plugins?.entries?.clickclack?.enabled).toBe(true);
    expect(onResolvedPlugin).toHaveBeenCalledTimes(1);
  });
  ...
  it("preserves accepted external install metadata when later channel setup goes back", async () => {
    ...
    const onPluginInstalled = vi.fn(async (cfg: OpenClawConfig) => cfg);
    ...
    expect(onResolvedPlugin).not.toHaveBeenCalled();
    expect(onAccountId).not.toHaveBeenCalled();
    expect(onPostWriteHook).not.toHaveBeenCalled();
  });

  it("rolls back bundled catalog enablement when channel setup goes back", async () => {
    const configure = vi.fn(async ({ cfg, prompter }) => {
      await prompter.text({ message: "ClickClack token" });
      return { cfg };
    });
    ...
    const bundledCatalogEntry = makeCatalogEntry("clickclack", "ClickClack", {
      origin: "bundled",
    });
    resolveChannelSetupEntries.mockReturnValue(
      makeChannelSetupEntries({
        entries: [{ id: "clickclack", meta: makeMeta("clickclack", "ClickClack") }],
        installableCatalogEntries: [bundledCatalogEntry],
        installableCatalogById: new Map([["clickclack", bundledCatalogEntry]]),
      }),
    );
    loadChannelSetupPluginRegistrySnapshotForChannel.mockReturnValue(
      makePluginRegistry({
    ...
    ensureChannelSetupPluginInstalled.mockImplementationOnce(async (params) => ({
      cfg: {
        ...
        plugins: {
          ...params.cfg.plugins,
          entries: { ...params.cfg.plugins?.entries, clickclack: { enabled: true } },
        },
      },
      installed: true,
      pluginId: "clickclack",
    ...
    const initialConfig = { plugins: { allow: ["memory-core"] } } as OpenClawConfig;
    ...
    const result = await setupChannels(
      ...
      {} as never,
      {
        confirm: vi.fn(async () => true),
        note: vi.fn(async () => undefined),
        select: vi.fn().mockResolvedValueOnce("clickclack").mockResolvedValueOnce("__done__"),
        text: vi.fn(async () => {
          throw new WizardNavigationError("back");
        }),
      } as never,
